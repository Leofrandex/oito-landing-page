...
- **A2. Integración del copy** ← SIGUIENTE. Aplicar el deck v2 a los componentes construidos usando §14 como checklist (WhyOito, CasesShowcase roster 3+1, Faq: quitar pregunta dual + añadir la de desarrollo, FinalCTA, Hero subtítulo/TextType).
...
- **C. Iniciativa SEO** (sin empezar), simplificada a una sola URL: C1 refactor SSR (quitar `'use client'` → islas), C2 metadata/OG/canonical, C3 robots + sitemap, C4 JSON-LD, C5 CWV + on-page. Title orientado a automatización con IA. ⚠️ El diagnóstico de abajo quedó obsoleto tras la v5 (ya no hay LoadingScreen ni `'use client'` global): hacer auditoría fresca antes del spec.
